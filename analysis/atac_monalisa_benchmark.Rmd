---
title: "ATAC-seq MonaLisa benchmark"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/atacseq/monalisa/'

# Read in all output files from monaLisa for every dataset
monalisa.txts <- list.files("/da/dmp/cb/steinse5/projects/tf_tool_benchmark/data/atacseq/",
                          recursive = T, full.names = T, pattern = 'monaLisa_results.txt')
...
if(!file.exists(file.path(out.path, "monaLisa_benchmark.RDS"))) {
  ...
  monalisa.results <- lapply(monalisa.txts, readInMonalisaTxt)  %>% bind_rows
  
  monalisa.list <- list('using no background' = monalisa.results)
