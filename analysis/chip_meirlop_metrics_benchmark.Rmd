---
title: "ChIP-seq MEIRLOP benchmark using different metrics"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_metrics//meirlop/'
if(!dir.exists(out.path)) dir.create(out.path)

var.dict <- c('coef' = T, 'auc' = T, 'pval' = F, 'padj' = F, 'percent_peaks' = T)
...
readInMeirlopTxt <- function(meirlop.txt) {
  ...
  meirlop.results <- read_tsv(meirlop.txt, show_col_types = FALSE) %>%
    dplyr::rename(motif_name = 1)  %>% #, enrichment_coef = 2) %>%
...
if(!file.exists(file.path(out.path, "MEIRLOP_benchmark.RDS"))) {
  ...
  meirlop.list <- lapply(names(var.dict), function(x) meirlop.results) 
  names(meirlop.list) <- names(var.dict)
  ...
  for(meirlop.name in names(meirlop.list)) {
    l <- createSummaryDF(meirlop.list[[meirlop.name]],  meirlop.name, desc=var.dict[[meirlop.name]], 
                         lambert=TRUE)
