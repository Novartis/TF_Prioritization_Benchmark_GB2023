---
title: "ChIP-seq MEIRLOP benchmark (parameter optimisation)"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_parameters/meirlop/'

# Read in all knownResults.txt output files from meirlop for every dataset
meirlop.txts <- list.files("/da/dmp/cb/steinse5/projects/tf_tool_benchmark/data/chipseq/",
                          recursive = T, full.names = T, pattern = 'lr_results')
meirlop.txts <- meirlop.txts[grep(meirlop.txts, pattern = 'benchmark_parameters/.*/results/lr_results.*tsv$')]
...
if(!file.exists(file.path(out.path, "MEIRLOP_benchmark.RDS"))) {
  ...
  # Define parameters
  params.list <- list('kmer' = c('kmer0_gc', sprintf('kmer%s', 0:3)), 
                      'length' =  c('', 'length'))
 
  # Read MEIRLOP outputs 
  meirlop.list <- lapply(params.list[[1]], function(param1) {
    meirlop.list <- lapply(params.list[[2]], function(param2) {
      if(param2 == 'length') {
        p <- sprintf('%s_%s', param1, param2)
      } else {
        p <- sprintf('%s.tsv', param1)
      }
      meirlop.txt <- meirlop.txts[grep(meirlop.txts, pattern = p)]
      lapply(meirlop.txt, readInMeirlopTxt)  %>% bind_rows 
    })
    names(meirlop.list) <- params.list[[2]]
    return(meirlop.list)
  })
  names(meirlop.list) <- params.list[[1]]
  meirlop.list <- do.call(c, meirlop.list)
  names(meirlop.list) <- gsub('\\.', '_', names(meirlop.list))
