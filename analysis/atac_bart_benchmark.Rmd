...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/atacseq/bart/'

# Read in all knownResults.txt output files from bart for every dataset
bart.txts <- list.files("/da/dmp/cb/steinse5/projects/tf_tool_benchmark/data/atacseq/",
                          recursive = T, full.names = T, pattern = 'diffPeak_bart_results.txt')
...
if(!file.exists(file.path(out.path, "BART_benchmark.RDS"))) {
  ...
  bart.list <- list('using no background peaks' = bart.results)
