---
title: "ChIP-seq BART benchmark using different metrics"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_metrics/bart/'
...
bart.txts <- list.files("/da/dmp/cb/steinse5/projects/tf_tool_benchmark/data/chipseq/",
                        ...
                        pattern = 'diffPeak_bart_results.txt')


var.dict <- c('statistic' = T, 'pvalue' = F, 'zscore' = T,
              'max_auc' = T, 're_rank' = F, 'irwin_hall_pvalue' = F)
...
readInbartTxt <- function(bart.txt) {
  ...
  dplyr::rename(motif_name = 1) %>% # p_value = 6) %>%
  mutate(study = conds[11], label = conds[16]) %>%
  dplyr::select(study, label, motif_name, names(var.dict))
...
if(!file.exists(file.path(out.path, "BART_benchmark.RDS"))) {
  ...
  bart.results <- lapply(bart.txts, readInbartTxt)  %>% bind_rows
  
  bart.list <- lapply(names(var.dict), function(x) bart.results) 
  names(bart.list) <- names(var.dict)
  ...
  for(bart.name in names(bart.list)) {
    ...
    l <- createSummaryDF(bart.list[[bart.name]], bart.name, desc=var.dict[[bart.name]], 
                         lambert=FALSE)
