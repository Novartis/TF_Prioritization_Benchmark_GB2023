---
title: "ChIP-seq - TFEA benchmark using different metrics"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_metrics//tfea/'
if(!dir.exists(out.path)) dir.create(out.path)

var.dict <- c("E-Score" = T, "Corrected E-Score" = T, 'Events' = T, 
...
if(!file.exists(file.path(out.path, "TFEA_benchmark.RDS"))) {
  ...
  tfea.list <- lapply(names(var.dict), function(x) tfea.results) 
  names(tfea.list) <- names(var.dict)
  ...
  for(tfea.name in names(tfea.list)) {
    l <- createSummaryDF(tfea.list[[tfea.name]], tfea.name, desc=var.dict[[tfea.name]], 
                         lambert=TRUE)
