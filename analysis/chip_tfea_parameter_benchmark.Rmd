---
title: "ChIP-seq - TFEA benchmark (parameter optimisation)"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_parameters/tfea/'
...
tfea.txts <- tfea.txts[!grepl("tmp_", tfea.txts)]
tfea.txts <- tfea.txts[grep(tfea.txts, pattern = 'benchmark_parameters/TFEA')]
...
if(!file.exists(file.path(out.path, "TFEA_benchmark.RDS"))) {
  ...
  # Define parameters
  params.list <- list('sw' = sprintf('sw%s', c(150, 300, 600)), 
                      'lw' = sprintf('lw%s', c(1500, 3000, 6000)))
 
  # Read MEIRLOP outputs 
  tfea.list <- lapply(params.list[[1]], function(param1) {
    tfea.list <- lapply(params.list[[2]], function(param2) {
      p <- sprintf('%s_%s', param1, param2)
      tfea.txt <- tfea.txts[grep(tfea.txts, pattern = p)]
      lapply(tfea.txt, readInTfeaTxt)  %>% bind_rows 
    })
    names(tfea.list) <- params.list[[2]]
    return(tfea.list)
  })
  names(tfea.list) <- params.list[[1]]
  tfea.list <- do.call(c, tfea.list)
  names(tfea.list) <- gsub('\\.', '_', names(tfea.list))
