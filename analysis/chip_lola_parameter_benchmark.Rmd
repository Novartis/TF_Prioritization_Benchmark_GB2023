---
title: "ChIP-seq LOLA benchmark (parameter optimisation)"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_parameters/lola/'
...
lola.txts <- list.files("/da/dmp/cb/steinse5/projects/tf_tool_benchmark/data/chipseq/",
                          recursive = T, full.names = T, pattern = 'lola')
lola.txts <- lola.txts[grep(lola.txts, pattern = 'txt$')]
lola.txts <- lola.txts[grep(lola.txts, pattern = 'benchmark_parameters')]
...
#  dplyr::select(study, label, motif_name, p_value, possible_ranks)
  dplyr::select(study, label, motif_name, rank, possible_ranks) %>%
...
if(!file.exists(file.path(out.path, "lola_benchmark.RDS"))) {
  ...
  # Define parameters
  params.list <- list('minov' = sprintf('minov%s', c(1, 10, 25, 50, 100)),
                      'redefineset' = sprintf('redefineset%s', c('True', 'False')))
 
  # Read lola outputs 
  lola.list <- lapply(params.list[[1]], function(param1) {
    lola.list <- lapply(params.list[[2]], function(param2) {
      lola.txt <- lola.txts[grep(lola.txts, pattern = sprintf('%s.*%s', param1, param2))]
      lapply(lola.txt, readInLolaTxt)  %>%
        bind_rows 
    })
    names(lola.list) <- params.list[[2]]
    return(lola.list)
  })
  names(lola.list) <- params.list[[1]]
  lola.list <- do.call(c, lola.list)
  names(lola.list) <- gsub('\\.', '_', names(lola.list))
  ...
  for(lola.name in names(lola.list)) {
    ...
    l <- createSummaryDF(lola.list[[lola.name]], "rank", desc=FALSE, 
                         lambert=FALSE)
