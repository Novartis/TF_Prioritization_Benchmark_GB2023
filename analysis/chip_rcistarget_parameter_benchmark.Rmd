---
title: "ChIP-seq - RcisTarget benchmark (parameter optimisation)"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_parameters/rcistarget/'
...
rcistarget.txts <- rcistarget.txts[grep(rcistarget.txts, pattern = 'benchmark_parameters')]
...
if(!file.exists(file.path(out.path, "RcisTarget_benchmark.RDS"))) {
  ...
  # Define parameters
  bgs <- c('with_background', 'without_background')
  params <- sprintf('aucmaxrank%s', c(0.0025, 0.005, 0.01, 0.05, 0.1))
 
  # Read rcistarget outputs 
  rcistarget.list <- lapply(bgs, function(bg) {
    rcis.list <- lapply(params, function(param) {
      rcis.txt <- rcistarget.txts[grep(rcistarget.txts, pattern = sprintf('%s.*%s', bg, param))]
      lapply(rcis.txt, readInrcistargetTxt)  %>%
        bind_rows %>%
        mutate(possible_ranks=max(possible_ranks))
    })
    names(rcis.list) <- params
    return(rcis.list)
  })
  names(rcistarget.list) <- bgs
  rcistarget.list <- do.call(c, rcistarget.list)
  names(rcistarget.list) <- gsub('\\.', '_', names(rcistarget.list))
