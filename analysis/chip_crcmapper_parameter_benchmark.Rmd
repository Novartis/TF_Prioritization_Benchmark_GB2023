---
title: "ChIP-seq CRCmapper benchmark (parameter optimisation)"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_parameters/crcmapper/'

# Read in all knownResults.txt output files from crc for every dataset
crc.txts <- list.files("/da/dmp/cb/steinse5/projects/tf_tool_benchmark/data/chipseq/",
...
if(!file.exists(file.path(out.path, "CRCmapper_benchmark.RDS"))) {
  ...
  # Define parameters
  params.list <- list('elength' = sprintf('elength%s', c(50, 100, 250, 500)),
                      'nmotif' = sprintf('nmotif%s', c(1, 2, 5, 10))) 
  
  # Read CRCMapper outputs 
  crc.list <- lapply(params.list[[1]], function(param1) {
    crc.list <- lapply(params.list[[2]], function(param2) {
      p <- sprintf('%s_%s', param1, param2)
      crc.txt <- crc.txts[grep(crc.txts, pattern = p)]
      lapply(crc.txt, readIncrcTxt)  %>% bind_rows %>%
        mutate(possible_ranks = max(possible_ranks))
    })
    names(crc.list) <- params.list[[2]]
    return(crc.list)
  })
  names(crc.list) <- params.list[[1]]
  crc.list <- do.call(c, crc.list)
  names(crc.list) <- gsub('\\.', '_', names(crc.list))
  ...
  for(crc.name in names(crc.list)) {
    ...
    crc.list[[crc.name]] <- crc.list[[crc.name]] %>%
      dplyr::select(c("stringent", "study", "label", "possible_ranks", all_of(crc.rankvar))) %>%
    ...
    l <- createSummaryDF(crc.list[[crc.name]], crc.rankvar, desc=TRUE, 
                         lambert=FALSE)
