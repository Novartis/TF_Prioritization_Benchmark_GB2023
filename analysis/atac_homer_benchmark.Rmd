---
title: "ATAC-seq Homer benchmark"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/atacseq/homer/'

# Read in all knownResults.txt output files from homer for every dataset
homer.txts <- list.files("/da/dmp/cb/steinse5/projects/tf_tool_benchmark/data/atacseq/",
...
if(!file.exists(file.path(out.path, "homer_benchmark.RDS"))) {
  ...
  homer.resultsNB <- lapply(homer.nb, readInHomerTxt)  %>% bind_rows
  homer.resultsBL <- lapply(homer.bL, readInHomerTxt)  %>% bind_rows
  homer.resultsNBL <- lapply(homer.nbL, readInHomerTxt)  %>% bind_rows
  homer.list <- list('using background peaks' = homer.resultsB, 
                     'using no background peaks' = homer.resultsNB,
                     'using background peaks and lambert motifs' = homer.resultsBL,
                     'using no background peaks but lambert motifs' = homer.resultsNBL)
