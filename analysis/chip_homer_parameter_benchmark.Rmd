---
title: "ChIP-seq Homer benchmark (parameter optimisation)"
...
# Define output path
out.path <- '/da/dmp/cb/steinse5/projects/tf_tool_benchmark/output/benchmark_data/chipseq_parameters/homer/'
...
homer.txts <- list.files("/da/dmp/cb/steinse5/projects/tf_tool_benchmark/data/chipseq/",
                          recursive = T, full.names = T, pattern = 'knownResults_nlen.*.txt')
homer.txts <- homer.txts[grep(homer.txts, pattern = 'benchmark_parameters/homer')]
...
homer.nb <- homer.nL[grep(homer.nL, pattern = 'without_background')]
homer.bL <- homer.L[grep(homer.L, pattern = 'with_background')]
homer.nbL <- homer.L[grep(homer.L, pattern = 'without_background')]
...
if(!file.exists(file.path(out.path, "homer_benchmark.RDS"))) {
  
  # Define parameters
  params.list <- list('nlen' = sprintf('nlen%s', 0:3),
                      'gc' = c('gc', 'cpg', 'noweight'),
                      'test' = c('hyper', 'binom'))
  
  # Define HOMER TXTS
  homer.txts <- list('HOMER_bg' = homer.b, 
                     'HOMER' = homer.nb,
                     'HOMER_Lambert_bg' = homer.bL,
                     'HOMER_Lambert' = homer.nbL)
 
  # Read HOMER outputs 
  homer.list <- lapply(homer.txts, function(htxts) {
    homer.list <- lapply(params.list[[1]], function(param1) {
      homer.list <- lapply(params.list[[2]], function(param2) {
        homer.list <- lapply(params.list[[3]], function(param3) {
          p <- sprintf('%s_%s_%s', param1, param2, param3)
          htxt <- htxts[grep(htxts, pattern = p)]
          lapply(htxt, readInHomerTxt)  %>% bind_rows
        })
        names(homer.list) <- params.list[[3]]
        return(homer.list)
      })
      names(homer.list) <- params.list[[2]]
      homer.list <- do.call(c, homer.list)
      names(homer.list) <- gsub('\\.', '_', names(homer.list))
      return(homer.list)
    })
    names(homer.list) <- params.list[[1]]
    homer.list <- do.call(c, homer.list)
    names(homer.list) <- gsub('\\.', '_', names(homer.list))
    return(homer.list)
  })
  names(homer.list) <- names(homer.txts) 
  homer.list <- do.call(c, homer.list)
  names(homer.list) <- gsub('\\.', '_', names(homer.list))
  ...
  homer.list <- file.path(out.path, "homer_benchmark.RDS")
  top.ten <- file.path(out.path, "homer_topTen.RDS")
  roc.pr.str <- file.path(out.path, "homer_roc_pr_str.RDS")
  roc.pr.unstr <- file.path(out.path, "homer_roc_pr_unstr.RDS")
  info.df <- file.path(out.path, "homer_info.RDS")
  topMotifs.df <- file.path(out.path, "homer_topMotifs_lambert.RDS")
}
...
#
#The number of studies Homer without background succesfully run for: &emsp; `r length(homer.nb)` \
#
#The number of studies Homer without background included the studied transcription factor in their results at all: &emsp; `r nrow(homer.list[[2]])`
#
#The number of studies Homer with background but using Lambert succesfully run for: &emsp; `r length(homer.bL)` \
#
#The number of studies Homer with background but using Lambert included the studied transcription factor in their results at all: &emsp; `r nrow(homer.list[[3]])`
#
#The number of studies Homer without background and using Lambert succesfully run for: &emsp; `r length(homer.nbL)` \
#
#The number of studies Homer without background and using Lambert included the studied transcription factor in their results at all: &emsp; `r nrow(homer.list[[4]])`
#
...
if(!file.exists(file.path(out.path, "homer_benchmark.RDS"))) {
  # Save results as *.rds
  saveRDS(homer.list, file=file.path(out.path, "homer_benchmark.RDS"))
  saveRDS(top.ten, file=file.path(out.path, "homer_topTen.RDS"))
  saveRDS(roc.pr.str, file=file.path(out.path, "homer_roc_pr_str.RDS"))
  saveRDS(roc.pr.unstr, file=file.path(out.path, "homer_roc_pr_unstr.RDS"))
  saveRDS(info.df, file=file.path(out.path, "homer_info.RDS"))
  saveRDS(topMotifs.df, file=file.path(out.path, "homer_topMotifs_lambert.RDS"))
}
```
